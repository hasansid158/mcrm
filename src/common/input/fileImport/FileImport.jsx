...
const FileImport = ({
  ...
  maxSize = 5 * 1024 * 1024,
  allValidFiles = [],
  setAllValidFiles = (allFiles, newFiles) => {},
  handleFileDelete = () => {},
}) => {
  ...
  const errorMap = (fileName = '') => ({
    ...
    'file-too-large': `${fileName} - size is too large, only 5MB or less is accepted.`,
  ...
  const onDrop = (acceptedFiles, rejectedFiles) => {
    ...
    if (!_.isEmpty(rejectedFiles)) {
      ...
      errorCode && dispatch(setErrorDialogText(errorMap(firstFile?.file?.name)?.[errorCode] || 'Error while uploading file.'));
    }
  };
  ...
  return <>
    ...
    >
      ...
      <Box pb={isEmpty(allValidFiles) ? 0 : 3}>
        <Typography>Drag & drop {maxFiles > 1 ? 'files' : 'file'} here, or click to select {maxFiles > 1 ? 'files' : 'file'}</Typography>
        <Typography>(Accepted formats: {fileTypeArray?.join(' ')?.toUpperCase()})</Typography>
        {allValidFiles?.length >= 5 && <Typography color='red' variant='p2'>File upload limit exceeded (Max {maxFiles} uploads)</Typography>}
      </Box>
