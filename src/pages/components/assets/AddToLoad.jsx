...
const AddToLoad = ({
  ...
  const handleProjectChange = async (name, value) => {
    ...
    getLoadsList([value])
      ...
      .catch(err => {
        console.log(err);
        dispatch(setErrorDialogText('Error fetching loads, please try again later.'));
        setOpen(false);
      })
  ...
  const handleAttach = (data) => {
    ...
    attachAssetToLoad({
      ...
      .then(() => {
        const loadObject = find(loadList, { 'id': formData?.getValues?.('loadId') });
      ...
      .catch(err => {
        console.log(err);
        dispatch(setErrorDialogText('Error attaching loads, please try again later.'));
      })
      .finally(() => {
        setLoading(false);
        setOpen(false);
  ...
  return <>
    ...
    >
      ...
      >
        ...
        {!!loadList &&
          <Box display='flex' columnGap={2} rowGap={2} flexWrap='wrap' alignItems='center'>
