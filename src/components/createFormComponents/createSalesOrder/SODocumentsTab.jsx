...
const SODocumentsTab = ({
  ...
  useEffect(() => {
    ...
    setAcceptedFiles(saleOrderFiles);
  }, [saleOrderFiles, isUpdate]);

  const handleFileUpload = (allFiles, newFiles = []) => {
    if (isUpdate) {
      uploadAttachmentsSO(getFileFormData(newFiles), watchData?.salesOrderID)
        ?.then(res => {
          dispatch(setSnackBar({
            open: true,
            message: `${newFiles?.length} files uploaded to sale order successfully!`
          }));
