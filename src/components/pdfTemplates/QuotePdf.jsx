  ...
  <Document title="Quote-Invoice.pdf">
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.invoiceHeaderContainer}>
        ...
        <Text style={styles.text}>Contact Person: {data?.customerContactPersonName}</Text>
        <Text style={styles.text}>Phone: {data?.customerPhone}</Text>
        <Text style={styles.text}>Email: {data?.customerEmail}</Text>
        <Text style={styles.text}>Address: {data?.customerAddress} {data?.customerCity} {data?.customerState} {data?.customerZipCode} {data?.customerCountry}</Text>
      </View>
          </View>
        </View>
      ...
      <View style={styles.invoiceDetailsContainer}>
        ...
        <Text style={styles.text}>Quote Date: {new Date(data?.quoteDate).toLocaleDateString()}</Text>
        <Text style={styles.text}>Quote Expiry Date: {new Date(data?.quoteExpiryDate).toLocaleDateString()}</Text>
        ...
        <View style={styles.invoiceDetailsColumn}>
        <Text style={styles.text}>Unit Price: {data?.currency} {data?.unitPrice?.toFixed(2)}</Text>
        <Text style={styles.text}>Taxes: {data?.currency} {data?.taxes?.toFixed(2)}</Text>
        <Text style={styles.text}>Total Amount: {data?.currency} {data?.totalAmount?.toFixed(2)}</Text>
        <Text style={styles.text}>Discount: {data?.discount}%</Text>
        <Text style={styles.text}>Payment Method: {data?.paymentMethod}</Text>
        <Text style={styles.text}>Shipping Method: {data?.shippingMethod}</Text>
        </View>
        </View>
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.table}>
          <View style={[styles.tableRow, styles.tableHeader]}>
            ...
            <Text style={[styles.tableCol, styles.narrowCol]}>Unit Price</Text>
            <Text style={[styles.tableCol, styles.narrowCol]}>Total Amount</Text>
          </View>
          {data?.quoteItems?.map((item, key) => (
            <View key={key} style={styles.tableRow}>
              <Text style={[styles.tableCol, styles.narrowCol]}>{item?.quoteItemID}</Text>
              <Text style={[styles.tableCol, styles.wideCol]}>{item?.service}</Text>
              <Text style={[styles.tableCol, styles.wideCol]}>{item?.serviceDescription}</Text>
              <Text style={[styles.tableCol, styles.narrowCol]}>{item?.quantity}</Text>
              <Text style={[styles.tableCol, styles.narrowCol]}>{data?.currency} {item?.unitPrice?.toFixed(2)}</Text>
              <Text style={[styles.tableCol, styles.narrowCol]}>{data?.currency} {item?.totalAmount?.toFixed(2)}</Text>
            </View>
          ))}
        </View>
      </View>
